...
for filename in sorted(glob.glob(os.path.join(Binary_PATH, "*.png"))):
    try:
        segmentedImage = retina.Retina(
            None,
            filename,
            store_path=f"{AUTOMORPH_DATA}/Results/M2/binary_vessel/disc_centred_binary_process",
        )
...
for filename in sorted(glob.glob(os.path.join(Artery_PATH, "*.png"))):
    try:
        segmentedImage = retina.Retina(
            None,
            filename,
            store_path=f"{AUTOMORPH_DATA}/Results/M2/artery_vein/disc_centred_artery_process",
        )
        window_sizes = [912]
        window = retina.Window(
            segmentedImage, window_sizes[-1], min_pixels=CONFIG.pixels_per_window
...
for filename in sorted(glob.glob(os.path.join(Vein_PATH, "*.png"))):
    try:
        segmentedImage = retina.Retina(
            None,
            filename,
            store_path=f"{AUTOMORPH_DATA}/Results/M2/artery_vein/disc_centred_vein_process",
        )
        window_sizes = [912]
        window = retina.Window(
            segmentedImage, window_sizes[-1], min_pixels=CONFIG.pixels_per_window
...
Data4stage2.to_csv(
    f"{AUTOMORPH_DATA}/Results/M3/Disc_centred/Disc_Measurement.csv",
    index=None,
    encoding="utf8",
)
