            ...
            if center_img[i, j] == 255:
                ...
                while True:
                    ...
                    if image[i, j + w0 + 1] == 0 and image[i, j - w180 - 1] == 0:
                        ...
                        image[i - w45 - 1, j + w45 + 1] == 0
                        and image[i + w225 + 1, j - w225 - 1] == 0
                    ):
                        widths.append([i, j, 45, w45, w225])
                        break
                    elif (
                        image[i - w135 - 1, j - w135 - 1] == 0
                        and image[i + w315 + 1, j + w315 + 1] == 0
                    ):
...
def _preparing_data(
    ...
    if classified_img is not None:
        for w in widths:
            ...
            w1 = w[1]
            if (
                np.array_equal(classified_img[w0, w1], [255, 0, 0])
                or np.array_equal(classified_img[w0, w1], [0, 0, 255])
            ) and ((w[3] + w[4]) >= 2):
...
def _vector(
    ...
    if angle == 0:
        ...
        for i in range(0, sections + 1):
            ...
            iv.extend(
                [_local_binary_pattern(gray_img[w0 - 1 : w0 + 2, step - 1 : step + 2])]
            )
        iv.extend([w[3] + w[4]])
        iv.extend([out])
    elif angle == 45:
            ...
            iv.extend(
                [
                    _local_binary_pattern(
                        gray_img[stepx - 1 : stepx + 2, stepy - 1 : stepy + 2]
                    )
                ]
            )
        iv.extend([w[3] + w[4]])
        ...
        for i in range(0, sections + 1):
            ...
            iv.extend(
                [_local_binary_pattern(gray_img[step - 1 : step + 2, w1 - 1 : w1 + 2])]
            )
        iv.extend([w[3] + w[4]])
        iv.extend([out])
    elif angle == 135:
...
def _loading_model(
    original: np.ndarray, threshold: np.ndarray, av: np.ndarray, size: int
):
    ...
    for row in range(0, features.shape[0]):
        prediction = loaded_model.predict(
            np.divide(features[row : row + 1, 2:size], 255), batch_size=1
        )
    ...
    if av == 0:
        ...
        for i in range(0, 1000):
            ...
            for row in range(0, features.shape[0]):
                ...
                if (
                    int(predict_copy[features[row, 0], features[row, 1]]) == 1
                    and features[row, size] == 0
                ):
                    true_negative += 1
                elif (
                    int(predict_copy[features[row, 0], features[row, 1]]) == 2
                    and features[row, size] == 1
                ):
                    true_positive += 1
                elif (
                    int(predict_copy[features[row, 0], features[row, 1]]) == 2
...
def _average_width(
    ...
    for c in connected:
        ...
        connected_avg.extend(
            [
                _average(label_widths),
                final_image[formatted_indexes[index][0], formatted_indexes[index][1]],
            ]
        )
    ...
    for it_x in range(0, segmented_img.shape[0]):
        for it_y in range(0, segmented_img.shape[1]):
            if segmented_img[it_x, it_y] == 255:
                if (
                    all(gt_img[it_x, it_y] == [255, 0, 0])
                    or all(gt_img[it_x, it_y] == [0, 0, 255])
                ) and not (all(post_img[it_x, it_y] == [255, 255, 255])):
                    counter += 1
                    if all(post_img[it_x, it_y] == [0, 0, 255]) and all(
                        gt_img[it_x, it_y] == [0, 0, 255]
                    ):
                        TP += 1
                    elif all(post_img[it_x, it_y] == [255, 0, 0]) and all(
                        gt_img[it_x, it_y] == [255, 0, 0]
                    ):
                        TN += 1
                    elif all(post_img[it_x, it_y] == [0, 0, 255]) and all(
                        gt_img[it_x, it_y] == [255, 0, 0]
                    ):
                        FP += 1
                    elif all(post_img[it_x, it_y] == [255, 0, 0]) and all(
...
def classification(original_img: np.ndarray, manual_img: np.ndarray):
    ...
    acc, rgb, network, original = _validating_model(
        features, sectioned_img, original_img, predict_img, 38, 0
    )
    ...
    post_img = _postprocessing(
        connected_components, thr_img, bifurcations, img_original
    )
    return post_img
