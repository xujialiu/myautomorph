...
class Retina_grayscale(object):
    ...
    def opening(self, size_structure):
        ...
        """
        self._copy()
        self.np_image = ndimage.grey_opening(
            self.np_image, size=(size_structure, size_structure)
        )
    ...
    def closing(self, size_structure):
        ...
        """
        self._copy()
        self.np_image = ndimage.grey_closing(
            self.np_image, size=(size_structure, size_structure)
        )
    ...
    def top_hat(self, size_structuring_element):
        ...
        """
        self._copy()
        self.np_image = cv2.morphologyEx(
            self.np_image,
            cv2.MORPH_TOPHAT,
            cv2.getStructuringElement(
                cv2.MORPH_RECT, (size_structuring_element, size_structuring_element)
            ),
        )
    ...
    def mean_filter(self, structure):
        ...
        """
        self._copy()
        self.np_image = cv2.blur(
            self.np_image, (structure, structure)
        )  # signal.medfilt(self.np_image, structure)
    ...
    def median_filter(self, structure):
        ...
        """
        self._copy()
        self.np_image = cv2.medianBlur(
            self.np_image.astype(np.uint8), structure
        )  # ndimage.median_filter(self.np_image, size=(structure, structure))
    ...
    def shadow_correction(self):
        ...
        self.median_filter(
            self.kernel_median_filter
        )  # Ver si es posible aumentarlo en otro pc
    ...
    def normal_vessels_segmentation(self):
        ...
        npaContours, hierarchy = cv2.findContours(
            normal_vessels_segmentation, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        for npaContour in npaContours:
            if (
                cv2.contourArea(npaContour) < self.normal_vessels_segmentation_min_value
            ):  # 100
                cv2.drawContours(
                    normal_vessels_segmentation, [npaContour], -1, (255, 255, 255), -1
                )
    ...
    def tiny_vessels_segmentation(self):
        ...
        tiny_vessels_segmentation = cv2.adaptiveThreshold(
            IH,
            255,
            self.main_adaptative_method,
            cv2.THRESH_BINARY,
            self.tiny_vessels_threshold,
            2,
        )  # 13
        ...
        tiny_vessels_segmentation = cv2.erode(
            tiny_vessels_segmentation, kernel, iterations=1
        )

        npaContours, hierarchy = cv2.findContours(
            tiny_vessels_segmentation, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        for npaContour in npaContours:
            if (
                cv2.contourArea(npaContour) < self.tiny_vessels_segmentation_min_value
            ):  # pragma: no cover
                cv2.drawContours(
                    tiny_vessels_segmentation, [npaContour], -1, (0, 0, 0), -1
                )  # 250

        kernel = np.ones((self.kernel_dilate, self.kernel_dilate), np.uint8)
        tiny_vessels_segmentation = cv2.dilate(
            tiny_vessels_segmentation, kernel, iterations=1
        )
    ...
    def post_processing(self, final_vessels_segmentation):
        kernel = np.ones((self.kernel_dilate, self.kernel_dilate), np.uint8)
        final_vessels_segmentation = cv2.dilate(
            final_vessels_segmentation, kernel, iterations=1
        )
        final_vessels_segmentation = abs(255 - final_vessels_segmentation)

        npaContours, hierarchy = cv2.findContours(
            final_vessels_segmentation.astype(np.uint8),
            cv2.RETR_TREE,
        ...
        for npaContour in npaContours:
            ...
            if radius < self.postprocesing_segmentation_min_value:
                center = (int(x), int(y))
                cv2.drawContours(
                    final_vessels_segmentation, [npaContour], -1, (255, 255, 255), -1
                )
        ...
        for i in range(0, self.smoothing_curves_iterations):
            final_vessels_segmentation = cv2.medianBlur(
                final_vessels_segmentation.astype(np.uint8),
                self.smoothing_curves_kernel,
            )

        cv2.pyrDown(final_vessels_segmentation, final_vessels_segmentation)

        kernel = np.ones((self.kernel_erode, self.kernel_erode), np.uint8)
        final_vessels_segmentation = cv2.erode(
            final_vessels_segmentation, kernel, iterations=1
        )
    ...
    def double_segmentation(self):
        ...
        final_vessels_segmentation = tiny_vessels_segmentation.astype(
            np.float
        ) + normal_vessels_segmentation.astype(np.float)
