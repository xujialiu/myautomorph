...
if os.path.exists(
    f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_artery_skeleton/.ipynb_checkpoints"
):
    shutil.rmtree(
        f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_artery_skeleton/.ipynb_checkpoints"
    )
if os.path.exists(
    f"{AUTOMORPH_DATA}/Results/M2/binary_vessel/macular_Zone_B_centred_vein_skeleton/.ipynb_checkpoints"
):
    shutil.rmtree(
        f"{AUTOMORPH_DATA}/Results/M2/binary_vessel/macular_Zone_B_centred_vein_skeleton/.ipynb_checkpoints"
    )
if os.path.exists(
    f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_binary_skeleton/.ipynb_checkpoints"
):
    shutil.rmtree(
        f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_binary_skeleton/.ipynb_checkpoints"
    )
...
Artery_PATH = (
    f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_artery_skeleton"
)
Vein_PATH = (
    f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_vein_skeleton"
)
Binary_PATH = (
    f"{AUTOMORPH_DATA}/Results/M2/binary_vessel/macular_Zone_B_centred_binary_skeleton"
)
...
for filename in sorted(glob.glob(os.path.join(Binary_PATH, "*.png"))):
    try:
        segmentedImage = retina.Retina(
            None,
            filename,
            store_path=f"{AUTOMORPH_DATA}/Results/M2/binary_vessel/macular_Zone_B_centred_binary_process",
        )
...
for filename in sorted(glob.glob(os.path.join(Artery_PATH, "*.png"))):
    try:
        segmentedImage = retina.Retina(
            None,
            filename,
            store_path=f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_artery_process",
        )
        window_sizes = [912]
        window = retina.Window(
            segmentedImage, window_sizes[-1], min_pixels=CONFIG.pixels_per_window
...
for filename in sorted(glob.glob(os.path.join(Vein_PATH, "*.png"))):
    try:
        segmentedImage = retina.Retina(
            None,
            filename,
            store_path=f"{AUTOMORPH_DATA}/Results/M2/artery_vein/macular_Zone_B_centred_vein_process",
        )
        # segmentedImage.threshold_image()
        # segmentedImage.reshape_square()
        # window_sizes = segmentedImage.get_window_sizes()
...
Data4stage2.to_csv(
    f"{AUTOMORPH_DATA}/Results/M3/Macular_centred/Macular_Zone_B_Measurement.csv",
    index=None,
    encoding="utf8",
)
