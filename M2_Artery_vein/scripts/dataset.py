...
class LearningAVSegData(Dataset):
    ...
    def preprocess(self, pil_img, label, mask, dataset_name, img_size, train_or):
        ...
        if train_or:
            ...
            label_array = np.round(
                rotate(label_array, angle, axes=(0, 1), reshape=False, mode="nearest")
            )
    ...
    def __getitem__(self, i):
        ...
        if self.train_or:
            label = Image.open(label_file[0]).resize(
        ...
        img, label, mask = self.preprocess(
            img, label, mask, self.dataset_name, self.img_size, self.train_or
        )
        i += 1
        return {
            "name": idx,
